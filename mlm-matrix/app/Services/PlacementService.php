...
class PlacementService
{
    ...
    public function place(User $user, ?User $sponsor = null): Node
    {
        return DB::transaction(function () use ($user, $sponsor) {
            ...
            // Find the best position for the new user
            $position = $this->findBestPosition($sponsor, $width, $maxDepth);
    ...
    /**
     * Find the best position for a new user using BFS.
     */
    private function findBestPosition(User $sponsor, int $width, int $maxDepth): array
    {
        ...
        while (!empty($queue)) {
            ...
            // Check if current node has space for children
        ...
        // If no position found, place under the sponsor
